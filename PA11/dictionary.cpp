...
void copymeaning2(Node*node,Node*nnode){
    if(node==nullptr||node->meaning.meaning[0]=='\0')return;
...
Dictionary Dictionary::merge(const Dictionary& d2) const{
    ...
    auto merge=[&](Node* temproot, vector<int> container){
        ...
        if(d3.find_node(str.c_str())){
            if(d3.find_node(str.c_str())->meaning.meaning[0]=='\0'){
                copymeaning2(temproot,d3.find_node(str.c_str()));
            }
            return;
        }
...
Dictionary Dictionary::merge(Dictionary&& d2) const{
    ...
    auto check=[&](Node* temproot, vector<int> container){
        string str;
        vector<int>::iterator it=container.begin();
        while(it!=container.end()){
            str+=(char)(*it+97);
