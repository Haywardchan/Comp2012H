...
// int num_row_constraints={1,3,1,2,2,2};
// int num_col_constraints={2,1,3,2,2,2};
//test case
// num_rows=5;
// num_cols=5;
    ...
    for(int i = 0; i < max_col_constraint; i++){
        ...
        for (int j = 0; j < num_cols; j++){
            if(i < num_col_constraints[j]){
                // if(col_constraints[j][i]==-49)continue;
...
void recursive_perms(int row_idx, int block_idx, int positions[MAX_COL], int& num_pos, int& num_perms, int position_arr_perms[MAX_PERM][MAX_CONSTRAINT_NUM]){
    int arr[MAX_COL];
    // cout<<"test:"<<block_can_shift(row_idx, block_idx, positions, num_row_constraints[row_idx]);
...
void get_row_perms(int row_idx) {
    ...
    // num_rows=15;
    // num_cols=15;
    // int row_constraints[MAX_ROW][MAX_CONSTRAINT_NUM]={{2, 2, 2, -1},{3,1,5, -1},{6,6,-1},{5,3,-1},{5,1,-1},{3,2,-1},{4,3,1,-1},{2,2,-1},{2,3,-1},{4,1,-1},{7,1,3,-1},{7,5,-1},{4, 1, 2, -1},{1,4,4,-1},{4,1,1, -1}};
    // int col_constraints[MAX_COL][MAX_CONSTRAINT_NUM]={{3,3, -1},{3,4, -1},{6,2, -1},{5, 4, -1},{4, 3, -1},{3,1,6,-1},{4,7,-1},{2,4,2,-1},{3,2,-1},{1,1,-1},{4,3,-1},{5,2,1,2,-1},{3,1,3,1,-1},{3,5,-1},{3,4,-1}};

    // num_rows=6;
    // num_cols=6;
    // int row_constraints[MAX_ROW][MAX_CONSTRAINT_NUM]={{3, -1},{1,1, 2, -1},{2,-1},{1, 1, -1},{1,2,-1},{2,2, -1}};
    // int col_constraints[MAX_COL][MAX_CONSTRAINT_NUM]={{1, 2, -1},{1, -1},{2,1,1, -1},{1,1,-1},{3, 2, -1},{1, 2, -1}};
    // int num_row_constraints[MAX_ROW] = {1,3,1,2,2,2}, num_col_constraints[MAX_COL] = {2,1,3,2,2,2};//test
...
bool check_rows_valid(int num_complete_rows) {
    ...
    // int length[MAX_COL]={0};
    //check col constraints
    // for (int i = 0; i < num_cols; i++)
    // {
    //     for (int j = 0; j < num_col_constraints[i]; j++)
    //     {
    //         length[i]+=col_constraints[i][j];
    //     }
    //     length[i]+=num_col_constraints[i]-1;
    //     cout<<length[i]<<"\n";
    // }
    ...
    for (int j = 0; j < num_cols; j++)
    {
        for (int i = 0; i < num_complete_rows; i++)
        {
            ...
            // if(first_X&&length[i]>num_rows-i)return false;
            //check empty col constraint
            if(col_constraints[j][0]==-1&&count_X>0)return false;
