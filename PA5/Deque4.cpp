...
int size(const Deque& deque){
    if(deque.start.current==deque.end.current)return 0;
    else return (deque.ll_size)*8 -(deque.start.current-deque.start.first) - (deque.end.last-deque.end.current);
}
